...
NTSTATUS device_init(WDFDRIVER driver, WDFDEVICE &device) {
  ...
  auto device_init = WdfControlDeviceInitAllocate(
      driver, &SDDL_DEVOBJ_SYS_ALL_ADM_RWX_WORLD_RW_RES_R);
  if (!device_init) {
    status = STATUS_INSUFFICIENT_RESOURCES;
    KdPrint(("|LIBBLOCK|device_init|failed to allocate WDF control device: %d",
             status));
    return status;
  }

  WdfDeviceInitSetDeviceType(device_init, FILE_DEVICE_NETWORK);
  WdfDeviceInitSetIoType(device_init,
                         WdfDeviceIoBuffered); // 缓冲模式，类似DO_BUFFERED_IO
  status = WdfDeviceInitAssignName(device_init, &device_name);
  if (!NT_SUCCESS(status)) {
    KdPrint(
        ("|LIBBLOCK|device_init|failed to create WDF device name:%d", status));
    WdfDeviceInitFree(device_init);
    return status;
  }

  WDF_FILEOBJECT_CONFIG file_config;
  WDF_FILEOBJECT_CONFIG_INIT(&file_config, file_create, file_close,
                             file_cleanup);
  ...
  if (!NT_SUCCESS(status)) {
    KdPrint(("|LIBBLOCK|device_init|failed to create WDF control device: %d",
             status));
    WdfDeviceInitFree(device_init);
    return status;
  }
  ...
  if (!NT_SUCCESS(status)) {
    KdPrint(("|LIBBLOCK|device_init|failed to create default WDF queue: %d",
             status));
    WdfDeviceInitFree(device_init);
    return status;
  }
  ...
  if (!NT_SUCCESS(status)) {
    KdPrint(("|LIBBLOCK|device_init|failed to create device symbolic link: %d",
             status));
    WdfDeviceInitFree(device_init);
    return status;
  }
  ...
  KdPrint(("|LIBBLOCK|device_init|end."));
  return status;
}
