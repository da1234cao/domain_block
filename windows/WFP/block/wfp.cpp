...
static NTSTATUS register_callout(DEVICE_OBJECT *wdm_device) {
  ...
  if (!NT_SUCCESS(status)) {
    KdPrint(("|LIBREDIRECT|register_callout|Failed to register IPv4 callout, "
             "status 0x%08x",
             status));
    return status;
  }
  ...
  if (!NT_SUCCESS(status)) {
    KdPrint(("|LIBBLOCK|register_callout|Failed to register IPv4 callout, "
             "status 0x%08x",
             status));
    return status;
  }
...
static void unregister_callout() {
  ...
  if (!NT_SUCCESS(status)) {
    KdPrint(("|LIBBLOCK|unregister_callout|Failed to unregister IPv4 callout, "
             "status: 0x%08x",
             status));
  }
  ...
  if (!NT_SUCCESS(status)) {
    KdPrint(("|LIBBLOCK|unregister_callout|Failed to unregister IPv6 callout, "
             "status: 0x%08x",
             status));
  }
}
...
NTSTATUS wfp_init(PDEVICE_OBJECT dev_obj) {
  ...
  session.flags =
      FWPM_SESSION_FLAG_DYNAMIC; // session结束后自动销毁所有callout和filter
  auto status = FwpmEngineOpen(NULL, RPC_C_AUTHN_WINNT, NULL, &session,
                               &wfp_engine_handle);
  if (!NT_SUCCESS(status)) {
    return status;
  }
