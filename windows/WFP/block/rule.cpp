...
rule_t& reverse_rule(rule_t& rule) {
	if (rule.remote_addr_info.ip_version = 4) {
		...
		KdPrint(("|LIBBLOCK|reverse_rule|unknow ip version: %d", rule.remote_addr_info.ip_version));
	}
	return rule;
}
...
NTSTATUS add_rule(rule_t& rule) {
	...
	rule_node_t* node = (rule_node_t*)ExAllocatePool2(POOL_FLAG_NON_PAGED, sizeof(rule_node_t), WDF_BLOCK_TAG);
	if (node == NULL) {
	...
	KdPrint(("|LIBBLOCK|add_rule|add rule: domain is %s, action is %d", node->rule.remote_addr_info.domin_str, node->rule.action));
...
NTSTATUS del_rule(rule_t& rule) {
	...
	WdfWaitLockAcquire(rule_list_lock, nullptr);
	for (PLIST_ENTRY node = rule_list.Flink; node != &rule_list; node = node->Flink) {
...
NTSTATUS clear_rules()
{
	...
	while (IsListEmpty(&rule_list) == FALSE) {
		...
		rule_node_t* rule_node = CONTAINING_RECORD(node, rule_node_t, list_entry);
		KdPrint(("|LIBBLOCK|clear_rules|clear %s kernel rule.", rule_node->rule.remote_addr_info.domin_str));
	...
	for (PLIST_ENTRY node = rule_list.Flink; node != &rule_list; node = node->Flink) {
		...
		if (connect_info.ip_version == 4) {
			...
			for (int i = 0; i < rule_node->rule.remote_addr_info.v6.cnt; i++) {
				if (is_same_ipv6(connect_info.v6.remote_address, rule_node->rule.remote_addr_info.v6.domain_address[i])) {
					result = TRUE;
					KdPrint(("|LIBBLOCK|is_hint_rule|hint rule: ip is%x:%x:%x:%x:%x:%x:%x::%x, action is %d", FORMAT_ADDR6(connect_info.v6.remote_address), rule_node->rule.action));
					goto end;
				}
			}
		}
		else {
			KdPrint(("|LIBBLOCK|is_hint_rule|unknow ip version: %d", connect_info.ip_version == 4));
		}
	}
