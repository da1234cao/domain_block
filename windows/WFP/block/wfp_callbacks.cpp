...
void callout_classify(const FWPS_INCOMING_VALUES *inFixedValues,
  ...
  if (inFixedValues->layerId == FWPS_LAYER_ALE_FLOW_ESTABLISHED_V4) {
    conn.ip_version = 4;
    // FWP_UINT32: 主机序
    conn.v4.local_address = *reinterpret_cast<IN_ADDR *>(
        &inFixedValues
             ->incomingValue
                 [FWPS_FIELD_ALE_FLOW_ESTABLISHED_V4_IP_LOCAL_ADDRESS]
             .value.uint32);
    // FWP_UINT32
    conn.v4.remote_address = *reinterpret_cast<IN_ADDR *>(
        &inFixedValues
             ->incomingValue
    ...
    conn.v4.local_port =
        inFixedValues
            ->incomingValue[FWPS_FIELD_ALE_FLOW_ESTABLISHED_V4_IP_LOCAL_PORT]
            .value.uint16;
    // FWP_UINT16
    conn.v4.remote_port =
        inFixedValues
            ->incomingValue[FWPS_FIELD_ALE_FLOW_ESTABLISHED_V4_IP_REMOTE_PORT]
            .value.uint16;

    KdPrint(("|LIBBLOCK|callout_classify|IPv4, %d.%d.%d.%d:%hu --> "
             "%d.%d.%d.%d:%hu, PID: %lld",
    ...
    conn.v6.local_address = *reinterpret_cast<IN6_ADDR *>(
        inFixedValues
            ->incomingValue[FWPS_FIELD_ALE_FLOW_ESTABLISHED_V6_IP_LOCAL_ADDRESS]
            .value.byteArray16);
    // FWP_BYTE_ARRAY16_TYPE
    conn.v6.remote_address = *reinterpret_cast<IN6_ADDR *>(
        inFixedValues
            ->incomingValue
    ...
    conn.v6.local_port =
        inFixedValues
            ->incomingValue[FWPS_FIELD_ALE_FLOW_ESTABLISHED_V6_IP_LOCAL_PORT]
            .value.uint16;
    // FWP_UINT16
    conn.v6.remote_port =
        inFixedValues
            ->incomingValue[FWPS_FIELD_ALE_FLOW_ESTABLISHED_V6_IP_REMOTE_PORT]
            .value.uint16;

    // conn.v6.remote_scope_id = inMetaValues->remoteScopeId;

    KdPrint(("|LIBBLOCK|callout_classify|IPv6, %x:%x:%x:%x:%x:%x:%x::%x:%hu "
             "--> %x:%x:%x:%x:%x:%x:%x::%x:%hu, PID: %lld",
...
NTSTATUS NTAPI callout_notify(FWPS_CALLOUT_NOTIFY_TYPE notifyType,
  ...
  case FWPS_CALLOUT_NOTIFY_ADD_FILTER:
    KdPrint(
        ("|LIBBLOCK|callout_notify|A new filter has registered, filterId: %lld",
         filter->filterId));
    break;
  case FWPS_CALLOUT_NOTIFY_DELETE_FILTER:
    // 删除一个filter时，将对应链表中相应的规则清楚
    KdPrint(("|LIBBLOCK|callout_notify|A filter has just been deleted, "
             "filterId: %lld",
             filter->filterId));
    // tood!
    break;
...
void NTAPI callout_flow_delete(UINT16 layerId, UINT32 calloutId,
                               UINT64 flowContext) {
