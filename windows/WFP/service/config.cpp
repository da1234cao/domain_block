...
void config::parse_toml(boost::filesystem::path &cfg_path) {
  ...
  m_cfg["max_log_file_size"] = tlbs["log"]["max_file_size"].value<std::string>().value();
  m_cfg["max_log_files"] = tlbs["log"]["max_files"].value<std::string>().value();
